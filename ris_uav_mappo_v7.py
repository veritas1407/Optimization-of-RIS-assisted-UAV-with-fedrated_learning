...
v7 UPGRADES (v6 → v7):
 32. [PHYSICS]     RHO0 raised 1e-2 → 5e-2 for clearer RIS benefit visibility
 33. [NEW]         generate_iot_positions() — uniform/clustered/edge deployments
 34. [VIZ]         Learning curve MA window 20→50 for smoother plots
 35. [VIZ]         Trajectory: bigger markers, goal annotation, denser arrows
 36. [ANALYSIS]    EE vs L: powers-of-2 [4,8,16,32,64,128], log-2 x-axis
 37. [ANALYSIS]    EE vs P_max: % gain annotations, fixed tick labels
 38. [BUG FIX]     EE vs Devices: integer ticks, 350→500 sub-train eps
 39. [NEW]         plot_ee_per_uav() — per-UAV fairness + Jain's index
 40. [NEW]         plot_scenario_comparison() — 3 IoT deployment scenarios
 41. [VIZ]         Summary plot expanded 3×2→4×2 with panels (g)+(h)
...
os.makedirs("./mappo_results_v7", exist_ok=True)
...
def generate_iot_positions(n_devices, mode="uniform", seed=None):
    ...
    if mode == "uniform":
        ...
        raise ValueError(f"Unknown mode '{mode}'. Choose: uniform, clustered, edge")
    ...
    for ep in iterator:
        ...
        if ep % EVAL_EVERY == 0:
            ...
            if eval_ee > best_eval_ee:
                ...
                best_ep      = ep
                agent.save("./mappo_results_v7/mappo_best.pt")
        ...
        if ep % SAVE_EVERY == 0:
            agent.save(f"./mappo_results_v7/mappo_ep{ep}.pt")

    agent.save("./mappo_results_v7/mappo_final.pt")
    print(f"\nTraining complete.")
    print(f"  Final model  → ./mappo_results_v7/mappo_final.pt")
    print(f"  Best model   → ./mappo_results_v7/mappo_best.pt  (ep={best_ep}, EE={best_eval_ee:.6f})")

    # FIX 8: reload best model for plotting so plots use peak performance
    print(f"  Loading best model for evaluation & plots...")
    agent.load("./mappo_results_v7/mappo_best.pt")
...
def plot_learning_curve(ep_rewards, ep_mean_EE, eval_EE_history=None, best_ep=0):
    ...
    plt.tight_layout()
    plt.savefig("./mappo_results_v7/mappo_learning_curve.png", dpi=150)
    plt.close()
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
...
def plot_ee_vs_L(agent):
    ...
    plt.tight_layout()
    plt.savefig("./mappo_results_v7/mappo_ee_vs_L.png", dpi=150)
    plt.close()
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
...
def plot_ee_vs_speed(agent):
    ...
    ax.legend();  ax.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig("./mappo_results_v7/mappo_ee_vs_speed.png", dpi=150)
    plt.close()
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
    ...
    for i, (pv, em, en) in enumerate(zip(p_vals, ee_mappo, ee_no_ris)):
        if en > 0:
            ...
            ax.annotate(f'+{gain:.1f}%',
                        ...
                        fontweight='bold')
    plt.tight_layout()
    plt.savefig("./mappo_results_v7/mappo_ee_vs_pmax.png", dpi=150)
    plt.close()
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
...
def plot_ee_vs_devices(agent):
    ...
    plt.tight_layout()
    plt.savefig("./mappo_results_v7/mappo_ee_vs_devices.png", dpi=150)
    plt.close()
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
...
def plot_ee_per_uav(agent):
    ...
    plt.tight_layout()
    plt.savefig("./mappo_results_v7/mappo_ee_per_uav.png", dpi=150)
    plt.close()
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
...
def plot_scenario_comparison(agent):
    """
    Compare average EE across three IoT device deployment scenarios:
    uniform, clustered, and edge. Uses the trained v7 agent.
    """
    scenarios  = ["uniform", "clustered", "edge"]
    colors_sc  = ["#2563eb", "#dc2626", "#16a34a"]
    n_devices_list = [2, 3, 4, 5, 6]
    n_eval     = 5
    ...
    plt.tight_layout()
    plt.savefig("./mappo_results_v7/mappo_scenario_comparison.png", dpi=150)
    plt.close()
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
...
def plot_trajectories(agent):
    ...
    plt.tight_layout()
    plt.savefig("./mappo_results_v7/mappo_trajectory.png", dpi=150)
    plt.close()
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
...
def plot_all(ep_rewards, ep_mean_EE, eval_EE_history, agent):
    ...
    sc_vals = ["uniform", "clustered", "edge"]
    sc_colors = ["#2563eb", "#dc2626", "#16a34a"]
    for i, sc in enumerate(sc_vals):
        ...
        for n_dev in [2, 4, 6]:
            ...
            sc_ees.append(em)
        ax7.plot([2, 4, 6], sc_ees, marker='o', color=sc_colors[i],
                 lw=1.5, ms=5, label=sc.capitalize())
    ax7.set_xticks([2, 4, 6])
    ...
    fig.suptitle("MAPPO K=3 UAV Swarm — RIS-Assisted IoT Network Results (v7)",
                 fontsize=14, fontweight="bold", y=1.01)
    plt.savefig("./mappo_results_v7/mappo_summary.png", dpi=150, bbox_inches="tight")
    plt.close()
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
...
def main():
    ...
    print("\nGenerating new v7 plots...")
    plot_ee_per_uav(agent)
    plot_scenario_comparison(agent)
    print("  [OK] All v7 plots generated.")

    print(f"\nAll results saved to ./mappo_results_v7/")
