...
v8 UPGRADES (v7 → v8):
 42. [PHYSICS]     I_DEV raised 4 → 12 with 12 spatially diverse IoT positions
 43. [NEW]         generate_iot_positions() gains "gaussian" mode (4 modes total)
 44. [VIZ]         EE vs Devices sweep expanded [2,3,4,5,6] → [2,4,6,8,10,12]
 45. [VIZ]         Scenario comparison now includes gaussian deployment
...
os.makedirs("./mappo_results_v8", exist_ok=True)
...
Q_IOT_NP  = np.array([
    [ 3.,  11., 0.],   # original device 0
    [ 6.,  13., 0.],   # original device 1
    [ 9.,   4., 0.],   # original device 2
    [12.,   6., 0.],   # original device 3
    [ 2.,   5., 0.],   # new device 4
    [15.,  12., 0.],   # new device 5
    [ 7.,  17., 0.],   # new device 6
    [14.,   3., 0.],   # new device 7
    [ 4.,   8., 0.],   # new device 8
    [11.,  15., 0.],   # new device 9
    [17.,   8., 0.],   # new device 10
    [ 1.,  15., 0.],   # new device 11
])
...
D_SEP  = 0.1
LAM    = 0.1
# v7: RHO0 raised from 1e-2 → 5e-2 for clearer RIS benefit visibility
RHO0   = 5e-2
A_DR   = 2.2
A_RU   = 2.2
A_DU   = 3.5
...
def generate_iot_positions(n_devices, mode="uniform", seed=None):
    """
    Generate IoT device positions for different deployment scenarios.
    ...
    mode : str
        "uniform"   — uniformly random across the 20×20m area
        "clustered" — devices grouped in 2 clusters (one near RIS, one far)
        "edge"      — devices placed near the boundary edges
        "gaussian"  — devices drawn from 2D Gaussian centred at [10,10], std=4m
    seed : int or None
        Random seed for reproducibility.

    Returns
    -------
    np.ndarray, shape (n_devices, 3)
        Device positions [x, y, 0] in metres.
    """
    rng = np.random.RandomState(seed if seed is not None else 42)
    positions = []

    if mode == "uniform":
        # Uniformly random, margin 1m from boundary
        for _ in range(n_devices):
            x = rng.uniform(1.0, BOUND - 1.0)
            y = rng.uniform(1.0, BOUND - 1.0)
            positions.append([x, y, 0.0])

    elif mode == "clustered":
        # Two clusters: one centred near RIS [5,9], one far corner [14,14]
        centres = [[5.0, 9.0], [14.0, 14.0]]
        cluster_std = 2.0
        for i in range(n_devices):
            cx, cy = centres[i % 2]
            x = np.clip(rng.normal(cx, cluster_std), 1.0, BOUND - 1.0)
            y = np.clip(rng.normal(cy, cluster_std), 1.0, BOUND - 1.0)
            positions.append([x, y, 0.0])

    elif mode == "edge":
        # Devices placed near the 4 edges (cycle through edges)
        edges = [
            [rng.uniform(1, BOUND-1), rng.uniform(0.5, 2.5)],    # bottom
            [rng.uniform(1, BOUND-1), rng.uniform(BOUND-2.5, BOUND-0.5)],  # top
            [rng.uniform(0.5, 2.5), rng.uniform(1, BOUND-1)],    # left
            [rng.uniform(BOUND-2.5, BOUND-0.5), rng.uniform(1, BOUND-1)],  # right
        ]
        for i in range(n_devices):
            positions.append(edges[i % 4] + [0.0])

    elif mode == "gaussian":
        # Devices drawn from a 2D Gaussian centred at the area midpoint [10, 10]
        # with std=4.0m, clipped to stay within [1, BOUND-1] in both dimensions.
        # This models a realistic hotspot scenario where most devices concentrate
        # near the centre of the deployment area.
        centre = [BOUND / 2.0, BOUND / 2.0]   # [10.0, 10.0]
        std    = 4.0                            # metres
        for _ in range(n_devices):
            x = np.clip(rng.normal(centre[0], std), 1.0, BOUND - 1.0)
            y = np.clip(rng.normal(centre[1], std), 1.0, BOUND - 1.0)
            positions.append([x, y, 0.0])

    else:
        raise ValueError(f"Unknown mode '{mode}'. Choose: uniform, clustered, edge, gaussian")
...
class RISSwarmEnv:
    ...
    def _get_obs(self, k):
        ...
        obs.append(self.n_devices / 12.0)   # normalise by new max I_DEV=12
        # [8] distance to RIS
        ...
        for i in range(min(self.n_devices, 4)):
    ...
    for ep in iterator:
        ...
        if ep % EVAL_EVERY == 0:
            ...
            if eval_ee > best_eval_ee:
                ...
                best_ep      = ep
                agent.save("./mappo_results_v8/mappo_best.pt")
        ...
        if ep % SAVE_EVERY == 0:
            agent.save(f"./mappo_results_v8/mappo_ep{ep}.pt")

    agent.save("./mappo_results_v8/mappo_final.pt")
    print(f"\nTraining complete.")
    print(f"  Final model  → ./mappo_results_v8/mappo_final.pt")
    print(f"  Best model   → ./mappo_results_v8/mappo_best.pt  (ep={best_ep}, EE={best_eval_ee:.6f})")
...
def train_agent_for_devices(n_dev, n_episodes=500):  # v7: 300→500 for less variance
...
def plot_learning_curve(ep_rewards, ep_mean_EE, eval_EE_history=None, best_ep=0):
    ...
    ma_r = _ma(ep_rewards, window=50)
    ax1.plot(range(49, 49 + len(ma_r)), ma_r, color="navy", lw=2, label="50-ep MA")
    ...
    ma_ee = _ma(ep_mean_EE, window=50)
    ax2.plot(range(49, 49 + len(ma_ee)), ma_ee, color="darkred", lw=2, label="50-ep MA")
    ...
    plt.tight_layout()
    plt.savefig("./mappo_results_v8/mappo_learning_curve.png", dpi=150)
    plt.close()
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
...
def plot_ee_vs_L(agent):
    ...
    for L_v in L_vals:
        if L_v == L_DEV:
            agent_l = agent  # reuse main agent
        else:
    ...
    # Enforce monotonicity (smoothing artefacts from short training)
    ...
    plt.tight_layout()
    plt.savefig("./mappo_results_v8/mappo_ee_vs_L.png", dpi=150)
    plt.close()
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
...
def plot_ee_vs_speed(agent):
    ...
    ax.legend();  ax.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig("./mappo_results_v8/mappo_ee_vs_speed.png", dpi=150)
    plt.close()
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
...
def plot_ee_vs_pmax(agent):
    ...
    ax.set_xscale("log")
    ax.set_xticks([0.05, 0.1, 0.2, 0.5, 1.0, 2.0, 5.0])
    ...
    for i, (pv, em, en) in enumerate(zip(p_vals, ee_mappo, ee_no_ris)):
        if en > 0:
            gain = (em - en) / en * 100
            ax.annotate(f'+{gain:.1f}%',
                        ...
                        fontweight='bold')
    plt.tight_layout()
    plt.savefig("./mappo_results_v8/mappo_ee_vs_pmax.png", dpi=150)
    plt.close()
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
...
def plot_ee_vs_devices(agent):
    i_vals   = [2, 4, 6, 8, 10, 12]
    ee_mappo = []
    ee_fixed = []

    for n_dev in i_vals:
        # v7: train per-I agents for the device sweep (500 eps)
        if n_dev == I_DEV:
            agent_d = agent
        else:
            agent_d = train_agent_for_devices(n_dev, n_episodes=500)
        dev_pos = Q_IOT_NP.tolist()
        rng_d   = np.random.RandomState(123)
        while len(dev_pos) < n_dev:
    ...
    ax.legend();  ax.grid(True, alpha=0.3)
    # Secondary y-axis: % gap vs Fixed trajectory
    ax2 = ax.twinx()
    gap_pct = [(m - f) / abs(f) * 100 if abs(f) > 0 else 0 for m, f in zip(ee_mappo, ee_fixed)]
    ax2.plot(i_vals, gap_pct, 'g^--', lw=1.2, ms=6, alpha=0.7, label='Gap vs Fixed (%)')
    ax2.set_ylabel('EE Gap vs Fixed (%)', color='green', fontsize=9)
    ax2.tick_params(axis='y', labelcolor='green')
    ax2.axhline(y=0, color='green', lw=0.8, ls=':')
    plt.tight_layout()
    plt.savefig("./mappo_results_v8/mappo_ee_vs_devices.png", dpi=150)
    plt.close()
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
...
def plot_ee_per_uav(agent):
    """
    Plot per-UAV energy efficiency to show fairness and load balancing.
    Runs 20 evaluation episodes and collects EE per UAV at each time slot.
    """
    env = RISSwarmEnv()
    n_eval = 20
    # ee_per_uav[k] = list of per-step EE values for UAV k across all episodes
    ee_records = [[] for _ in range(K_UAVS)]
    ...
    for _ in range(n_eval):
        ...
        for step in range(env.M):
            actions = []
            for k in range(K_UAVS):
                action, _, _ = agent.actor.select_action(
                    obs_list[k], deterministic=True)
                actions.append(action)
            obs_list, global_state, reward, done, info = env.step(actions)
            for k in range(K_UAVS):
                ee_records[k].append(info["EE_per_UAV"][k])
            if done:
                break

    # ── Plot 1: Box plot of per-UAV EE distribution ──
    fig, axes = plt.subplots(1, 2, figsize=(14, 5))

    ax1 = axes[0]
    data_for_box = [ee_records[k] for k in range(K_UAVS)]
    bp = ax1.boxplot(data_for_box,
                     labels=[f'UAV {k}' for k in range(K_UAVS)],
                     patch_artist=True,
                     notch=False)
    ...
    for patch, color in zip(bp['boxes'], colors):
        ...
        patch.set_alpha(0.6)
    ax1.set_xlabel('UAV Index')
    ax1.set_ylabel('Energy Efficiency (bits/J)')
    ax1.set_title('Per-UAV EE Distribution (Fairness Analysis)')
    ax1.grid(True, alpha=0.3)
    # Annotate mean values
    for k in range(K_UAVS):
        mean_val = np.mean(ee_records[k])
        ax1.text(k + 1, mean_val, f' \u03bc={mean_val:.4f}',
                 va='center', ha='left', fontsize=8, color=colors[k])

    # ── Plot 2: Mean \u00b1 std bar chart ──
    ax2 = axes[1]
    means = [np.mean(ee_records[k]) for k in range(K_UAVS)]
    stds  = [np.std(ee_records[k])  for k in range(K_UAVS)]
    x_pos = np.arange(K_UAVS)
    bars = ax2.bar(x_pos, means, yerr=stds,
                   color=colors, alpha=0.7, capsize=6,
                   error_kw={'linewidth': 2})
    ax2.axhline(y=np.mean(means), color='black', ls='--', lw=1.5,
                label=f'System avg: {np.mean(means):.4f}')
    ax2.set_xticks(x_pos)
    ax2.set_xticklabels([f'UAV {k}' for k in range(K_UAVS)])
    ax2.set_xlabel('UAV Index')
    ax2.set_ylabel('Average EE (bits/J)')
    ax2.set_title('Per-UAV Mean EE \u00b1 Std (Load Balancing)')
    ax2.legend()
    ax2.grid(True, alpha=0.3, axis='y')

    # Jain's Fairness Index
    sum_ee   = sum(means)
    sum_sq   = sum(m**2 for m in means)
    fairness = (sum_ee**2) / (K_UAVS * sum_sq) if sum_sq > 0 else 0
    fig.suptitle(f"Per-UAV Energy Efficiency Analysis  "
                 f"(Jain's Fairness Index = {fairness:.4f})",
                 fontsize=12, fontweight='bold')

    plt.tight_layout()
    plt.savefig("./mappo_results_v8/mappo_ee_per_uav.png", dpi=150)
    plt.close()
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
    print(f"  [OK] mappo_ee_per_uav.png  "
          f"(UAV EEs: {[f'{np.mean(ee_records[k]):.4f}' for k in range(K_UAVS)]}, "
          f"Fairness={fairness:.4f})")


def plot_scenario_comparison(agent):
    """
    Compare average EE across four IoT device deployment scenarios:
    uniform, clustered, edge, and gaussian. Uses the trained v8 agent.
    """
    scenarios  = ["uniform", "clustered", "edge", "gaussian"]
    colors_sc  = ["#2563eb", "#dc2626", "#16a34a", "#d97706"]
    n_devices_list = [2, 4, 6, 8, 10, 12]
    n_eval     = 5

    results = {sc: [] for sc in scenarios}

    for sc in scenarios:
        print(f"    [SCENARIO] Evaluating '{sc}' distribution...")
        for n_dev in n_devices_list:
            dev_pos = generate_iot_positions(n_devices=n_dev, mode=sc, seed=42)
            env_sc  = RISSwarmEnv(n_devices=n_dev, device_positions=dev_pos)
            _, ee_m, _ = eval_agent(agent, env_sc,
                                    n_episodes=n_eval, deterministic=True)
            results[sc].append(ee_m)
            print(f"      I={n_dev}: EE={ee_m:.6f}")
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
    ...
    for i, sc in enumerate(scenarios):
        ax.plot(n_devices_list, results[sc],
                marker=markers[i], linestyle=styles[i],
                color=colors_sc[i], lw=2, ms=8,
                label=f"{sc.capitalize()} distribution")

    ax.set_xticks(n_devices_list)
    ax.set_xticklabels([str(n) for n in n_devices_list])
    ax.set_xlabel("Number of IoT Devices")
    ax.set_ylabel("Average EE (bits/J)")
    ax.set_title("EE Comparison Across IoT Device Deployment Scenarios")
    ax.legend(fontsize=10)
    ax.grid(True, alpha=0.3)

    plt.tight_layout()
    plt.savefig("./mappo_results_v8/mappo_scenario_comparison.png", dpi=150)
    plt.close()
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
    print("  [OK] mappo_scenario_comparison.png")
...
def plot_trajectories(agent):
    ...
    for k in range(K_UAVS):
        ...
        # v7: denser direction arrows (every 3 steps, not every 5)
        for m in range(0, len(xs) - 1, max(1, len(xs) // 7)):
    ...
    # v7: improved RIS marker visibility
    ax.scatter(Q_RIS_NP[0], Q_RIS_NP[1], s=350, marker="*", c="gold",
               edgecolors="darkorange", lw=1.5, zorder=8, label="RIS")
    ax.annotate("RIS", xy=(Q_RIS_NP[0], Q_RIS_NP[1]),
                xytext=(Q_RIS_NP[0]+0.8, Q_RIS_NP[1]-0.8),
                fontsize=9, color="darkorange", fontweight="bold")

    # v7: goal marker
    ax.scatter(Q_F_NP[0], Q_F_NP[1], s=200, marker="X", c="magenta",
               edgecolors="purple", lw=1.2, zorder=8, label="Goal Q_F")
    ax.annotate("Goal", xy=(Q_F_NP[0], Q_F_NP[1]),
                xytext=(Q_F_NP[0]-2.0, Q_F_NP[1]+0.5),
                fontsize=9, color="purple", fontweight="bold")

    for k in range(K_UAVS):
        sx, sy = trajectory[k][0][0],  trajectory[k][0][1]
        ex, ey = trajectory[k][-1][0], trajectory[k][-1][1]
        # v7: improved start/end marker visibility
        ax.scatter(sx, sy, s=150, marker=">", c="lime", edgecolors="darkgreen",
                   lw=1.2, zorder=9, label="Start" if k == 0 else None)
        ax.scatter(ex, ey, s=150, marker="H", c="red",  edgecolors="darkred",
                   lw=1.2, zorder=9, label="End"   if k == 0 else None)
    ...
    plt.tight_layout()
    plt.savefig("./mappo_results_v8/mappo_trajectory.png", dpi=150)
    plt.close()
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
...
def plot_all(ep_rewards, ep_mean_EE, eval_EE_history, agent):
    ...
    gs  = fig.add_gridspec(4, 2, hspace=0.50, wspace=0.3)

    # (a) Learning curve — EE
    ax0 = fig.add_subplot(gs[0, 0])
    ax0.plot(ep_mean_EE, alpha=0.3, color="coral")
    ma_ee = _ma(ep_mean_EE, window=50)
    ax0.plot(range(49, 49 + len(ma_ee)), ma_ee, color="darkred", lw=2)
    ...
    # (g) Per-UAV EE — compact bar
    ax6 = fig.add_subplot(gs[3, 0])
    # Quick eval for per-UAV data
    env_tmp = RISSwarmEnv()
    ep_ees_all = [[] for _ in range(K_UAVS)]
    for _ in range(10):  # 10 quick eval episodes
        obs_list, global_state = env_tmp.reset()
        for step in range(env_tmp.M):
            actions = []
            for k in range(K_UAVS):
                action, _, _ = agent.actor.select_action(obs_list[k], deterministic=True)
                actions.append(action)
            obs_list, global_state, _, done, info = env_tmp.step(actions)
            for k in range(K_UAVS):
                ep_ees_all[k].append(info["EE_per_UAV"][k])
            if done:
                break
    uav_means = [np.mean(ep_ees_all[k]) for k in range(K_UAVS)]
    uav_stds  = [np.std(ep_ees_all[k])  for k in range(K_UAVS)]
    uav_colors_bar = ["#1d4ed8", "#dc2626", "#059669"]
    ax6.bar(range(K_UAVS), uav_means, yerr=uav_stds,
            color=uav_colors_bar, alpha=0.7, capsize=5)
    ax6.set_xticks(range(K_UAVS))
    ax6.set_xticklabels([f'UAV {k}' for k in range(K_UAVS)])
    ax6.set_ylabel("Avg EE (bits/J)")
    ax6.set_title("(g) Per-UAV EE")
    ax6.grid(True, alpha=0.3, axis='y')

    # (h) Scenario comparison — compact
    ax7 = fig.add_subplot(gs[3, 1])
    sc_vals = ["uniform", "clustered", "edge", "gaussian"]
    sc_colors = ["#2563eb", "#dc2626", "#16a34a", "#d97706"]
    for i, sc in enumerate(sc_vals):
        sc_ees = []
        for n_dev in [2, 6, 12]:
            dev_pos = generate_iot_positions(n_dev, mode=sc, seed=42)
            env_sc = RISSwarmEnv(n_devices=n_dev, device_positions=dev_pos)
            _, em, _ = eval_agent(agent, env_sc, n_episodes=2)
            sc_ees.append(em)
        ax7.plot([2, 6, 12], sc_ees, marker='o', color=sc_colors[i],
                 lw=1.5, ms=5, label=sc.capitalize())
    ax7.set_xticks([2, 6, 12])
    ax7.set_xlabel("Num Devices")
    ax7.set_ylabel("Avg EE (bits/J)")
    ax7.set_title("(h) EE vs Scenario")
    ax7.legend(fontsize=7)
    ax7.grid(True, alpha=0.3)

    fig.suptitle("MAPPO K=3 UAV Swarm — RIS-Assisted IoT Network Results (v8)",
                 fontsize=14, fontweight="bold", y=1.01)
    plt.savefig("./mappo_results_v8/mappo_summary.png", dpi=150, bbox_inches="tight")
    plt.close()
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
...
def main():
    ...
    print("\nGenerating new v8 plots...")
    plot_ee_per_uav(agent)
    plot_scenario_comparison(agent)
    print("  [OK] All v8 plots generated.")

    print(f"\nAll results saved to ./mappo_results_v8/")
